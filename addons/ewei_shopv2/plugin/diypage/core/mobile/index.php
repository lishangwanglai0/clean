...
class Index_EweiShopV2Page extends PluginMobilePage 
{
	...
	public function getInfo() 
	{
		...
		if( $urlData[0] == "goodsids" || $urlData[0] == "category" || $urlData[0] == "groups" ) 
		{
			...
			{
				$pcate = $urlValue[0];
				$goodsql = "SELECT id,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,nocommission,`type`,ispresell,`virtual`,hasoption,video FROM " . tablename("ewei_shop_goods") . " WHERE FIND_IN_SET(" . $pcate . ",cates) AND status > 0 AND deleted = 0 AND uniacid =" . $_W["uniacid"] . " limit 0," . $goodsnum;
